...
def funcerror_uncorr(func,x0,xerr,**kwargs):
    ''' Propagate uncertainties in function parameters to uncertainty in 
    function value. The uncertainty will be evaluated at f(x0). The method uses 
    ...
    input:
        func : any function
        x0   : a list or tuple containing function parameters
        xerr : a lits or tuple containing uncertainty in each parameter
    output:
        ferr : the uncertainty in f(x0)
    '''
    ...
    # We choose a larger value to for smaller roundoff error. Note 
