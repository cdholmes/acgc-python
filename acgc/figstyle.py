...
Additional colormaps and fonts will be loaded.
...
The additional colormaps are from Fabio Crameri's Scientific color maps 
...
def load_crameri_colormaps(noprefix=True):
    ...
    Parameters
    ----------
    noprefix : bool, default=True
        If True, remove the 'cmc.' prefix from the colormap names
    '''

    # Import the colormaps; The registered names will be prefixed with 'cmc.'
    try:
        from cmcrameri import cm
    except ImportError:
        warnings.warn('Scientific colormaps not found. Install with `conda install cmcrameri` if desired.', ImportWarning)
        return

    # Remove the 'cmc.' prefix from the colormap names
    for name in list(mpl.colormaps):
        if name[:4]=='cmc.':

            # Register the colormap without the prefix
            try:
                mpl.colormaps.register( mpl.colormaps[name], name=name[4:] )
            except ValueError:
                pass
            
            # Unregister the original colormap
            if noprefix:
